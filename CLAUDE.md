...
- Rhino startup uses a serialized temporary HKCU registration as its only plug-in loading mechanism (ADR 0012). The overlay writes the complete registration Rhino needs to load the selected `.rhp` at startup, never only a file name. Never also pass the `.rhp` on Rhino's command line: that asks Rhino to install an ID the overlay has already registered, and Rhino rejects it as an ID already in use. Restore every current-user value the launch wrote before launch returns; never require elevation or modify HKLM.
