...
Launch mode belongs to the invoking adapter. The desktop footer publishes one button per mode and passes the one pressed; the project-specific default from Config is what the worktree row's mode chip reports and what Enter on the list and the CLI pass. The MCP server likewise publishes separate Build & Launch and Launch Existing tools so an agent chooses explicitly for each request. MCP launch tools do not inherit that default. Both remain inside the same RWL launch and loaded-binary verification path.
