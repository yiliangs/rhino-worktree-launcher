...
The selected Git worktree is the single source and build tree. Each registered project stores one canonical Rhino plug-in project, Visual Studio solution, solution Configuration, Platform, and launch mode.
...
- Direct Launch is fast but may load a stale artifact; the UI states that it does not rebuild.
...
- The app-owned catalog still stores the selection, grants, and launch mode; remote mirrors, launch verification state, and logs remain app-owned.
