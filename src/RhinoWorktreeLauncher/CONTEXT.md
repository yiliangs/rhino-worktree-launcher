...
Launch is explicit through the button or Enter key. Double-click does not launch. The button waits for the backend's app-owned build, process start, and loaded-binary verifier result; it must never infer success from `Process.Start`.
