...
`Assets/rhino-launcher.png` is the header mark and `Assets/rhino-launcher.ico` is the executable and taskbar icon. The fixed 720 × 1000 interface uses bundled IBM Plex Sans and Geist Mono fonts. Worktree rows show upstream divergence and latest-commit activity; missing upstream metadata degrades to quiet zero values rather than blocking discovery.
